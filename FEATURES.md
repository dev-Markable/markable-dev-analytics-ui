...
| PR | Performance Review (досье + команды) | ▰▰▰ | ▰▰▰ | ✅ | да |
| TM | Команды first-class (1.7.0) + лиды | ▰▰▰ | ▰▰▰ | ✅ | да |
| KI | Kaiten-инсайты в perf-review (1.8.0) | ▰▰▰ | ▰▰ | ✅ | да |
...
**Готово** (контракт `@devpulse-dev/api-types@1.3.0`, бэк, фронт):

- [x] `getHourly({from, to, email?})` → `HourlyStats`, стор `useHourlyStore` (через `createStatsStore`, TTL-кэш)
- [x] `buildHourlyGrid` — достройка разреженных ячеек до полной сетки 7×24, пик, maxCommits
- [x] `HourlyHeatmap` widget — grid день×час, color-mix-шкала (темо-адаптивная), tooltip, легенда, подпись пика
- [x] секция на Activity (командный агрегат)
- [x] +8 юнит-тестов (`buildHourlyGrid`, `intensityLevel`)
